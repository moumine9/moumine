...
import { isAuthenticated as checkAuth } from '../../utils/visitorAuth';
...
export default function Home() {
  ...
  const [authed, setAuthed] = useState(typeof window !== 'undefined' ? checkAuth() : false);
    ...
    <div>
      <div class="cover-container d-flex col-10 vh-100 p-3 mx-auto flex-column">
        ...
        <div class={"row my-5"}>
          <Profile class={"col-4"} />
          <div class={"col-8"}>
            <LinksElements authenticated={authed} onAuthenticated={(n, e) => setAuthed(true)} />
...
function LinksElements(props: {
  ...
  const generateValue = (value: string, key: string) => {
    ...
    return ["github", "email", "cellNumber", "linkedIn"].includes(key)
  ...
  return (
    <div>
      <ul class={`d-flex flex-row flex-wrap gap-2 ${props?.class}`}>
        {Object.entries(linksJson).map(([key, value]) => (
          <li key={`linksElements${key}`} className="d-flex p-3 flex-column align-items-center" data-toggle="tooltip" data-placement="top" title={key}>
